...
@app.blob_trigger(
    arg_name="blob", path="tracking", connection="STORAGE_CONNECTION_STRING"
)
def main(blob: func.InputStream):
    ...
    # the dawn of time.
    timestamp = datetime.strptime(data["timestamp"], "%d/%m/%Y %I:%M:%S %p").astimezone(
        TZ
    )
    threshold = int(
        os.environ.get("BLOB_PROCESS_THRESHOLD", 1440)
    )  # Default to a threshold of one day.
    since = datetime.now().astimezone(TZ) - timedelta(minutes=threshold)
    if timestamp < since:
        return
